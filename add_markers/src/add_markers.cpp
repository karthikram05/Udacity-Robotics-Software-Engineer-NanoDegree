...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    if (state == 0)
    {
      ...
      double pickup_distance = abs(robot_x_ - goto_X1) + abs(robot_y_ - goto_Y1);

      if (pickup_distance > DISTANCE_THRESHOLD)
      {
        marker.action = visualization_msgs::Marker::ADD;
        marker.pose.position.x = goto_X1;
        marker.pose.position.y = goto_Y1;
	marker.pose.position.z = 1.0;
      }
      else
      {
    ...
    {
      // Calculate manhattan distance
      double dropoff_distance = abs(robot_x_ - goto_X2) + abs(robot_y_ - goto_Y2);
      ...
      {
        marker.action = visualization_msgs::Marker::ADD;
        marker.pose.position.x = goto_X2;
        marker.pose.position.y = goto_Y2;
	marker.pose.position.z = 1.0;      }
    }
